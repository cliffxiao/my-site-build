...
class ModMsgField extends Module {
    ...
    public function admin_list() {
		...
		$fields=  MsgField::findAll2(""," order by i_order");
	...
	public function admin_edit() {
       $this->_layout = 'content';
        ...
        if ($cat_id == 0) {
		 ...
		 $curr_field = new MsgField($cat_id);
		if (!$curr_field) {
				$this->assign('json', Toolkit::jsonERR(__('Invalid ID!')));
				return '_error';
		}
	   ...
	   $act=ParamHolder::get('act', 'add');
        try {
			...
			if($act=='add'){
				...
				unset($field_info['id']);
				$max_order=MsgField::getMaxOrder();
	...
	public function admin_order() {
		...
		try {
			...
			foreach($order_info as $key => $val) {
				$sql = "update ".Config::$tbl_prefix."msg_fields set `i_order`='".mysql_escape_string($val)."' where id='".mysql_escape_string($key)."' ";
				$db->query($sql);
			}
		} catch (Exception $ex) {
			$this->assign('json', Toolkit::jsonERR($ex->getMessage()));
	...
	public function admin_delete() {
	   ...
	   $field_id=	intval($field_id);
	    try {
			$curr_field = new MsgField($field_id);
			$curr_field->delete();
